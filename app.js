...
function App(){
  ...
  const reste=totalRevenus-totalFixed-totalVariable-totalExcep;
  ...
  const restColor = reste>0?"#19A979":reste<0?"#C8516C":"#6C8893";
  ...
  return el("div",{style:S.app},
    ...
    el("div",{style:S.flowCard},
      ...
      flowRow("house","#E8743B","Fixes","− "+fmt(totalFixed),"#E8743B"),
      flowRow("repeat","#F2B53C","Variables","− "+fmt(totalVariable),"#F2B53C"),
      flowRow("zap","#945ECF","Exceptionnelles","− "+fmt(totalExcep),"#945ECF"),
      el("div",{style:S.flowDivider}),
      el("div",{style:{...S.resteBox,background:`linear-gradient(135deg,${restColor},${restColor}cc)`}},
        el("div",{style:S.resteLabel},el(Icon,{name:"piggy-bank",size:16,color:"#fff"})," ",reste>=0?"Disponible à épargner":"Découvert prévu"),
        el("div",{style:S.resteVal},fmt(reste)),
        reste>0 && el("div",{style:S.resteFoot},`${fmt(totalSaved)} en cagnottes · ${nonAffecte>0?fmt(nonAffecte)+" non affectés":"tout affecté ✓"}`))),
      ...
      el("div",{style:S.potList}, pots.map(p=>{
        ...
        return el("div",{key:p.id,style:S.potCard},
          el("div",{style:S.potTop},
            ...
            el("button",{style:S.delBtn,onClick:()=>delPot(p.id)},el(Icon,{name:"trash-2",size:13}))),
...
function PotModal({onClose,onSave}){
  const [label,setLabel]=useState(""); const [goal,setGoal]=useState(""); const [color,setColor]=useState(POT_PALETTE[0]);
  const submit=()=>{if(!label)return;onSave({label,goal:parseFloat(goal)||0,color});};
  return el(Modal,{title:"Nouvelle cagnotte",onClose},
    ...
    el("button",{style:S.saveBtn,onClick:submit},"Créer la cagnotte"));
}
...
const S = {
  ...
  input:{width:"100%",padding:"11px 13px",borderRadius:11,border:"1.5px solid #e2e6ee",fontSize:15,outline:"none",background:"#fbfcfe"},
