...
function MonthMovements({deposits,pots,monthLabel,delDeposit}){
  ...
  var ins=deposits.filter(function(d){return d.amount>0;});
  var outs=deposits.filter(function(d){return d.amount<0;});
  var totalIn=ins.reduce(function(s,d){return s+d.amount;},0);
  var totalOut=outs.reduce(function(s,d){return s+d.amount;},0);
  function renderDep(d){
    var p=pots.find(function(x){return x.id===d.potId;});
    var isW=d.amount<0;
    var amtColor=isW?"#C8516C":((p&&p.color)||"var(--text-3)");
    var sub=[];
    if(d.note) sub.push(el("span",{key:"note",style:{fontSize:11,color:"var(--text-3)",fontStyle:"italic"}},d.note));
  ...
  return el("div",{style:{marginTop:14}},
    el("button",{style:{...S.depHead,display:"flex",alignItems:"center",gap:6,background:"none",border:"none",padding:0,cursor:"pointer",width:"100%"},onClick:function(){setOpen(!open);}},
      "Mouvements de "+monthLabel,
      totalIn>0&&el("span",{style:{fontSize:11,fontWeight:700,color:"#19A979",background:"#19A97915",borderRadius:6,padding:"1px 7px",marginLeft:6}},"+"+fmt(totalIn)),
      totalOut<0&&el("span",{style:{fontSize:11,fontWeight:700,color:"#C8516C",background:"#C8516C12",borderRadius:6,padding:"1px 7px",marginLeft:4}},"−"+fmt(-totalOut)),
    ...
    open&&el(React.Fragment,null,
      ins.length>0&&el("div",{style:{marginTop:10}},
        el("div",{style:{fontSize:11,fontWeight:700,color:"#19A979",marginBottom:6,display:"flex",alignItems:"center",gap:5}},el(Icon,{name:"arrow-down-circle",size:12,color:"#19A979"}),"Versements ce mois"),
        ins.map(renderDep)),
      outs.length>0&&el("div",{style:{marginTop:ins.length>0?12:10}},
        el("div",{style:{fontSize:11,fontWeight:700,color:"#C8516C",marginBottom:6,display:"flex",alignItems:"center",gap:5}},el(Icon,{name:"arrow-up-circle",size:12,color:"#C8516C"}),"Retraits ce mois"),
        outs.map(renderDep))));
}
    ...
    view==="cagnottes"&&el(React.Fragment,null,
      ...
      el("div",{style:S.section},
        ...
        el("div",{style:S.potList},pots.map(function(p){
          ...
          var thisMoDeposits=(data.deposits||[]).filter(function(d){return d.potId===p.id;});
          var thisMoIn=thisMoDeposits.filter(function(d){return d.amount>0;}).reduce(function(a,d){return a+d.amount;},0);
          var thisMoOut=thisMoDeposits.filter(function(d){return d.amount<0;}).reduce(function(a,d){return a+d.amount;},0);
          ...
          var metaParts=[];
          if(thisMoIn>0) metaParts.push("+ "+fmt(thisMoIn)+" versés ce mois");
          if(thisMoOut<0) metaParts.push("− "+fmt(-thisMoOut)+" retirés ce mois");
...
function App(){
  ...
  useEffect(function(){
    ...
    var txdata=p.get("txdata")||"";
    if(txdata){
      var decoded="";
      try{ decoded=decodeURIComponent(txdata); }catch(e){ decoded=txdata; }
      var parsed=parseTransactions(decoded);
      if(parsed.length>0){ setTab("budget"); setModal({kind:"import",prefill:decoded}); }
      return;
    }
    var amt=parseFloat(p.get("amount")||"");
    var lbl=p.get("label")||"";
    if(amt>0){ setModal({kind:"quickadd",amount:amt,label:lbl}); }
  },[loaded]);
  useEffect(function(){
    if(!db) return;
  ...
  const addWithdrawal=(id,a,note,opts,potLabel)=>setMonthData(c=>{
    ...
    if(options.mode==="budget"){
      ...
      var covered=options.linkedExpenseCover||amt;
      var surplus=amt-covered;
      dep.linkedExpenseId=options.linkedExpenseId;
      dep.linkedExpenseCover=covered;
      dep.amount=-covered;
      ...
      for(var ci=0;ci<expCats.length;ci++){
        ...
        var newArr=arr.map(function(x){
          if(x.id===options.linkedExpenseId){
            found=true;
            var covers=(x.potCovers||[]).concat([{depId:depId,potId:id,potLabel:potLabel||"",coveredAmount:covered}]);
      ...
      // Le surplus (retrait > dépense) est un retrait libre qui augmente le non-affecté
      if(surplus>0){
        next.deposits=[...(c.deposits||[]),dep,{id:uid(),potId:id,amount:-surplus,note:note||"",surplusOf:depId}];
        return next;
      }
    }
  ...
  return el("div",{style:appStyle},
    ...
    isWide && (function(){
      var sideBtn=function(id,icon,label){
        ...
        return el("button",{key:id,style:Object.assign({},S.sideItem,on?S.sideItemOn:{}),onClick:function(){setTab(id);}},
          el(Icon,{name:icon,size:19,color:on?"var(--brand)":"var(--text-3)"}),label);
      };
      ...
      return el("div",{style:S.flowCard},
        ...
        el("div",{style:{marginTop:16}},
          el("div",{style:{display:"flex",height:12,borderRadius:7,overflow:"hidden",background:"var(--track)"}},
            flows.map(function(f,i){var w=Math.max(0,Math.round(f.v/of*100));return el("div",{key:i,className:"bar-grow",style:{width:w+"%",background:f.c}});})),
    ...
    !isWide && el("nav",{style:S.tabBar},
      navItems.map(function(t){
        ...
        return el("button",{key:id,style:Object.assign({},S.tabBtn,on?{color:"var(--brand)"}:{}),onClick:function(){setTab(id);}},
          el("span",{style:{display:"flex",transform:on?"scale(1.04)":"none",transition:"transform .2s cubic-bezier(.22,.61,.36,1)"}},
            el(Icon,{name:icon,size:23,color:on?"var(--brand)":"var(--text-3)"})),
          el("span",{style:{fontSize:10.5,fontWeight:on?700:500,letterSpacing:"-0.1px",color:on?"var(--brand)":"var(--text-3)",transition:"all .2s ease"}},label));
      })),
...
// Courbe lissée (contrôles horizontaux au milieu de chaque segment)
function smoothPath(pts){
  if(!pts.length) return "";
  if(pts.length===1) return "M"+pts[0][0]+" "+pts[0][1];
  var d="M"+pts[0][0].toFixed(1)+" "+pts[0][1].toFixed(1);
  for(var i=0;i<pts.length-1;i++){
    var a=pts[i],b=pts[i+1],cx=((a[0]+b[0])/2).toFixed(1);
...
function PotSparkline({months,potId,year,month,color}){
  var ABBR=["J","F","M","A","M","J","J","A","S","O","N","D"];
  // solde cumulé (évolution) sur 6 mois
  var vals=[],labels=[],cum=0;
  for(var i=5;i>=0;i--){
    ...
    cum+=t; vals.push(cum); labels.push(ABBR[d.getMonth()]);
  }
  var hasMove=vals.some(function(v){return v!==0;});
  if(!hasMove) return null;
  var W=220,H=46,pad=4;
  var mn=Math.min.apply(null,vals),mx=Math.max.apply(null,vals),range=(mx-mn)||1;
  var pts=vals.map(function(v,i){return [pad+(W-2*pad)*(i/(vals.length-1)),pad+(H-2*pad)*(1-(v-mn)/range)];});
  var line=smoothPath(pts);
  var area=line+" L"+pts[pts.length-1][0].toFixed(1)+" "+(H-pad)+" L"+pts[0][0].toFixed(1)+" "+(H-pad)+" Z";
  var gid="ps"+potId;
  return el("div",{style:{marginTop:12}},
    el("div",{style:{fontSize:10,color:"var(--text-4)",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.4px",marginBottom:5}},"Évolution · 6 mois"),
    el("svg",{viewBox:"0 0 "+W+" "+(H+12),style:{width:"100%",overflow:"visible"},preserveAspectRatio:"none"},
      el("defs",null,el("linearGradient",{id:gid,x1:"0",y1:"0",x2:"0",y2:"1"},
        el("stop",{offset:"0%","stop-color":color,"stop-opacity":"0.32"}),
        el("stop",{offset:"100%","stop-color":color,"stop-opacity":"0"}))),
      el("path",{d:area,fill:"url(#"+gid+")",stroke:"none"}),
      el("path",{className:"spark-line",d:line,fill:"none",stroke:color,strokeWidth:"2.5",strokeLinecap:"round",strokeLinejoin:"round"}),
      el("circle",{cx:pts[pts.length-1][0],cy:pts[pts.length-1][1],r:"3",fill:color}),
      labels.map(function(l,i){return el("text",{key:i,x:pts[i][0],y:H+9,textAnchor:"middle",fontSize:8,fill:"var(--text-4)"},l);})));
}
...
function MiniDonut(p){
  ...
  return el("svg",{className:"ring-in",width:size,height:size,viewBox:"0 0 "+size+" "+size,style:{flexShrink:0}},
      ...
      return el("div",{key:it.id,style:{display:"flex",flexDirection:"column",gap:0}},
        ...
        it.potCovers&&it.potCovers.length>0&&el("div",{style:{margin:"-2px 0 6px 22px"}},
          it.potCovers.map(function(pc,pci){
            ...
            var net=Math.max(0,it.amount-totalCovers);
            var pct=it.amount>0?Math.min(100,Math.round(pc.coveredAmount/it.amount*100)):100;
            var isLast=pci===it.potCovers.length-1;
            var fullyPaid=isLast&&net===0;
            return el("div",{key:pc.depId,style:{marginBottom:4}},
              el("div",{style:{display:"flex",alignItems:"center",gap:5,marginBottom:3,flexWrap:"wrap"}},
                ...
                fullyPaid&&el("span",{style:{fontSize:10,fontWeight:700,color:"#19A979",background:"#19A97920",borderRadius:5,padding:"0 5px"}},"✓ soldé")),
              el("div",{style:{height:4,background:"var(--border-2)",borderRadius:3,overflow:"hidden",maxWidth:160}},
                el("div",{style:{height:"100%",width:pct+"%",background:"#19A979",borderRadius:3,transition:"width .4s ease"}})),
              isLast&&!fullyPaid&&el("div",{style:{fontSize:10.5,color:"var(--text-3)",marginTop:2}},fmt(net)+" € restent à ta charge"));
          })));
    })),
...
function WithdrawModal({pot,expenses,onClose,onSave}){
  ...
  var coveredAmt=selExp?Math.min(amt,selExp.amount):amt;
  var netAmt=selExp?Math.max(0,selExp.amount-coveredAmt):0;
  var surplusAmt=selExp?Math.max(0,amt-selExp.amount):0;
  ...
  const submit=function(){
    ...
    var opts;
    if(mode==="expense"){
      opts={mode:"expense",linkedExpenseId:selectedExpId,linkedExpenseCover:coveredAmt,surplus:surplusAmt};
    } else {
      opts={mode:"simple"};
    }
  ...
  return el(Modal,{title:"Retirer de « "+pot.potLabel+" »",onClose},
    ...
    mode==="expense"&&el("div",{style:{marginBottom:12}},
      ...
      selExp&&amt>0&&el("div",{style:{marginTop:10,background:"#19A97910",borderRadius:10,padding:"10px 12px"}},
        ...
        surplusAmt>0&&el("div",{style:{marginTop:10,background:"#E8743B12",borderRadius:8,padding:"8px 10px",display:"flex",gap:8,alignItems:"flex-start"}},
          el(Icon,{name:"zap",size:14,color:"#E8743B"}),
          el("div",{style:{fontSize:12,color:"var(--text-2)",lineHeight:1.5}},
            el("strong",{style:{color:"#E8743B"}},fmt(surplusAmt)+" € en surplus"),
            " — le retrait dépasse la dépense. Ce surplus sera disponible dans ton budget libre (non-affecté).")))),
...
// Trouve la ligne de budget qui correspond le mieux à un libellé de transaction
function bestLineMatch(label,lines){
  var n=normLabel(label);
  if(!n) return null;
  var toks=n.split(" ").filter(function(t){return t.length>2;});
  var best=null,bestScore=0;
  for(var i=0;i<lines.length;i++){
    var ln=normLabel(lines[i].label);
    if(!ln) continue;
    var lt=ln.split(" ").filter(function(t){return t.length>2;});
    var score=0;
    if(n.indexOf(ln)>=0||ln.indexOf(n)>=0) score+=2;
    toks.forEach(function(t){ if(ln.indexOf(t)>=0) score+=1; });
    lt.forEach(function(t){ if(n.indexOf(t)>=0) score+=1; });
...
function ImportModal(props){
  ...
  if(step===1) return el(Modal,{title:"📋 Importer mon relevé",onClose:onClose},
    ...
    el("button",{style:Object.assign({},S.saveBtn,{marginTop:14}),onClick:parse},"Analyser les opérations"));
  ...
  return el(Modal,{title:"📋 "+txs.length+" opérations détectées",onClose:onClose},
    ...
    el("div",{style:{fontSize:12.5,color:"var(--text-3)",marginBottom:10}},
      mode==="reel"?(matchedCount+" / "+selected.length+" rapprochées · "+fmt(total)):(selected.length+" sélectionnées · "+fmt(total))),
        ...
        return el("div",{key:t.id,style:{background:t.checked?"var(--surface)":"var(--surface-2)",borderRadius:12,padding:"10px 12px",border:"1px solid "+(t.checked?(mode==="reel"&&!t.matchId?"var(--border-3)":"var(--border)"):"var(--border-2)"),opacity:t.checked?1:0.5}},
          ...
          t.checked&&mode==="reel"&&el("div",{style:{display:"flex",alignItems:"center",gap:8,paddingLeft:28}},
            ...
            el("select",{value:t.matchId||"",onChange:function(e){setMatch(t.id,e.target.value);},style:{flex:1,minWidth:0,padding:"6px 8px",borderRadius:8,border:"1px solid var(--border)",background:"var(--field-bg)",color:"var(--text)",fontSize:12.5}},
              el("option",{value:""},"— ignorer —"),
              lines.map(function(l){return el("option",{key:l.id,value:l.id},l.label);}))));
      })),
    mode==="reel"
      ? (matchedCount>0
          ? el("button",{style:S.saveBtn,onClick:doReel},"Renseigner le réel · "+matchedCount+" ligne"+(matchedCount>1?"s":""))
          : el("button",{style:Object.assign({},S.saveBtn,{background:"var(--surface-3)",color:"var(--text-3)",boxShadow:"none"}),disabled:true},"Rapproche au moins une opération"))
      : (selected.length>0
...
// ============================================================================
// Calculs fiscaux & financiers centralisés dans fiscal.js (purs & testés — voir fiscal.test.js)
var IR_BRACKETS = Fiscal.IR_BRACKETS;
var IR_PLAFOND_DEMI_PART = Fiscal.IR_PLAFOND_DEMI_PART;
var irDecote = Fiscal.irDecote;
var irParts = Fiscal.irParts;
var irTaxOnQuotient = Fiscal.irTaxOnQuotient;
var irMarginalRate = Fiscal.irMarginalRate;
var loanMonthly = Fiscal.loanMonthly;
var loanCapacity = Fiscal.loanCapacity;
...
// ---- Bilan annuel (plein écran) ----
function BilanAnnuel({months,year,categories,onChangeYear,onClose}){
  categories = categories || [];
  ...
  var moMax = b.byMonth.reduce(function(m,x){ return Math.max(m,x.revenus,x.depenses); }, 1);
  var moNet = b.byMonth.reduce(function(s,x){ return s + x.net; }, 0);

  var kpi=function(label,val,color,sub){
    return el("div",{style:{background:"var(--surface)",border:"1px solid var(--line,var(--border))",borderRadius:16,padding:"16px 18px",boxShadow:"var(--shadow-card)"}},
      el("div",{style:{fontSize:11.5,color:"var(--text-3)",fontWeight:600,marginBottom:6}},label),
      el("div",{style:{fontSize:24,fontWeight:800,letterSpacing:"-0.5px",color:color}},val),
      sub?el("div",{style:{fontSize:11.5,color:"var(--text-3)",marginTop:3}},sub):null);
  };
  return el("div",{style:{position:"fixed",inset:0,zIndex:120,background:"var(--bg-gradient,var(--bg))",overflowY:"auto",WebkitOverflowScrolling:"touch"}},
    el("div",{style:{maxWidth:900,margin:"0 auto",padding:"calc(20px + env(safe-area-inset-top)) 18px calc(40px + env(safe-area-inset-bottom))",display:"flex",flexDirection:"column",gap:18}},
      // header
      el("div",{style:{display:"flex",alignItems:"center",gap:12}},
        ...
        el("div",{style:{flex:1}},
          el("div",{style:{fontSize:12,color:"var(--text-3)",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.5px"}},"Bilan annuel"),
          el("div",{style:{fontSize:24,fontWeight:800,letterSpacing:"-0.5px"}},"Année "+year)),
        el("div",{style:{display:"flex",gap:6}},
          el("button",{onClick:function(){onChangeYear(-1);},style:{border:"none",background:"var(--surface-2)",borderRadius:10,width:38,height:38,cursor:"pointer",color:"var(--text)"}},el(Icon,{name:"chevron-left",size:18})),
          el("button",{onClick:function(){onChangeYear(1);},style:{border:"none",background:"var(--surface-2)",borderRadius:10,width:38,height:38,cursor:"pointer",color:"var(--text)"}},el(Icon,{name:"chevron-right",size:18})))),
      // KPIs
      el("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12}},
        kpi("Revenus","+ "+fmt(b.revenus),"#19A979","sur l'année"),
        kpi("Dépenses","− "+fmt(b.depenses),"#C8516C","nettes"),
        kpi("Épargne",fmt(b.epargne),b.epargne>=0?"#1D8BCE":"#C8516C","mise de côté"),
        kpi("Taux d'épargne",tauxEpargne+" %",tauxEpargne>=10?"#19A979":"#E8743B","des revenus")),
      // Évolution mensuelle
      el("div",{style:{background:"var(--surface)",borderRadius:18,padding:18,boxShadow:"var(--shadow-card)"}},
        el("div",{style:{fontSize:13,fontWeight:800,color:"var(--text-2)",textTransform:"uppercase",letterSpacing:"0.4px",marginBottom:14}},"Revenus vs dépenses, mois par mois"),
        el("div",{style:{position:"relative",height:150,paddingBottom:20}},
          [0.25,0.5,0.75].map(function(f,i){return el("div",{key:i,style:{position:"absolute",left:0,right:0,top:(130*(1-f))+"px",height:1,background:"var(--border-2)"}});}),
          el("div",{style:{position:"relative",display:"flex",alignItems:"flex-end",gap:"2.5%",height:130}},
            b.byMonth.map(function(x){
              var rh=Math.round(x.revenus/moMax*118), dh=Math.round(x.depenses/moMax*118);
              return el("div",{key:x.month,style:{flex:1,display:"flex",flexDirection:"column",alignItems:"center",height:"100%",justifyContent:"flex-end"}},
                el("div",{style:{display:"flex",gap:3,alignItems:"flex-end",height:118}},
                  el("div",{className:"bar-grow-y",title:"Revenus "+fmt(x.revenus),style:{width:8,height:Math.max(2,rh),background:"linear-gradient(180deg,#22c088,#13a06f)",borderRadius:"3px 3px 0 0"}}),
                  el("div",{className:"bar-grow-y",title:"Dépenses "+fmt(x.depenses),style:{width:8,height:Math.max(2,dh),background:"linear-gradient(180deg,#ef8047,#d0632e)",borderRadius:"3px 3px 0 0"}})),
                el("div",{style:{fontSize:9,color:"var(--text-3)",marginTop:6}},MONTHS_FR[x.month-1].slice(0,1)));
            }))),
        el("div",{style:{display:"flex",gap:16,fontSize:11.5,color:"var(--text-3)",marginTop:6}},
          el("span",{style:{display:"flex",alignItems:"center",gap:5}},el("span",{style:{width:9,height:9,borderRadius:2,background:"#19A979"}}),"Revenus"),
          el("span",{style:{display:"flex",alignItems:"center",gap:5}},el("span",{style:{width:9,height:9,borderRadius:2,background:"#E8743B"}}),"Dépenses"),
          el("span",{style:{marginLeft:"auto",fontWeight:700,color:moNet>=0?"#19A979":"#C8516C"}},"Solde année : "+(moNet>=0?"+":"")+fmt(moNet)))),
      // Répartition par catégorie
      el("div",{style:{background:"var(--surface)",borderRadius:18,padding:18,boxShadow:"var(--shadow-card)"}},
        el("div",{style:{fontSize:13,fontWeight:800,color:"var(--text-2)",textTransform:"uppercase",letterSpacing:"0.4px",marginBottom:14}},"Où part l'argent — par catégorie"),
        catRows.length===0&&el("p",{style:{fontSize:13,color:"var(--text-3)",margin:0}},"Range tes dépenses en catégories (via la puce sous chaque ligne du budget) pour voir la répartition ici."),
        el("div",{style:{display:"flex",flexDirection:"column",gap:11}},catRows.map(function(r){
          var pct=b.depenses>0?Math.round(r.total/b.depenses*100):0;
          return el("div",{key:r.id},
            el("div",{style:{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:4}},
              el("span",{style:{display:"flex",alignItems:"center",gap:7,color:"var(--text-2)",fontWeight:600}},el("span",{style:{width:10,height:10,borderRadius:3,background:r.color}}),r.label),
              el("span",{style:{fontWeight:700}},fmt(r.total)+"  ·  "+pct+" %")),
            el("div",{style:{height:7,background:"var(--border-2)",borderRadius:4,overflow:"hidden"}},
              el("div",{style:{height:"100%",width:Math.round(r.total/catMax*100)+"%",background:r.color,borderRadius:4}})));
        })))));
}
...
function SubscriptionsCard(props){
  var data=props.data||{}, categories=props.categories||[];
  var subs=[];
  ["fixed","variable","excep"].forEach(function(k){
    (data[k]||[]).forEach(function(l){
      if(l.loanId) return;
      if(l.recurring || Core.catTop(l.cat,categories)==="abo"){ subs.push(l); }
    });
  });
  if(subs.length===0) return null;
  subs.sort(function(a,b){return (b.amount||0)-(a.amount||0);});
  var mens=subs.reduce(function(s,l){return s+(l.amount||0);},0);
  return el("div",{style:{background:"var(--surface)",borderRadius:20,padding:20,boxShadow:"var(--shadow-card)"}},
    el("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}},
      el("span",{style:{fontSize:13,fontWeight:800,color:"var(--text-2)",textTransform:"uppercase",letterSpacing:"0.4px",display:"flex",alignItems:"center",gap:8}},el(Icon,{name:"repeat",size:15,color:"#13A4B4"}),"Abonnements & récurrents"),
      el("span",{style:{fontSize:12,color:"var(--text-3)"}},subs.length)),
    el("div",{style:{display:"flex",gap:16,margin:"8px 0 14px"}},
      el("div",null,el("div",{style:{fontSize:22,fontWeight:800,color:"#13A4B4",letterSpacing:"-0.5px"}},fmt(mens)),el("div",{style:{fontSize:11,color:"var(--text-3)"}},"par mois")),
      el("div",null,el("div",{style:{fontSize:22,fontWeight:800,color:"var(--text)",letterSpacing:"-0.5px"}},fmt(mens*12)),el("div",{style:{fontSize:11,color:"var(--text-3)"}},"par an"))),
    el("div",{style:{display:"flex",flexDirection:"column",gap:7}},subs.slice(0,8).map(function(l){
      return el("div",{key:l.id,style:{display:"flex",alignItems:"center",justifyContent:"space-between",fontSize:13.5}},
        el("span",{style:{color:"var(--text-2)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginRight:10}},l.label||"—"),
        el("span",{style:{fontWeight:700,color:"var(--text)",flexShrink:0}},fmt(l.amount||0)));
    })));
}
...
function ForecastCard(props){
  ...
  return el("div",{style:{background:"var(--surface)",borderRadius:20,padding:compact?18:20,boxShadow:"var(--shadow-card)"}},
    ...
    el("svg",{viewBox:"0 0 "+W+" "+H,style:{width:"100%",height:H,display:"block"},preserveAspectRatio:"none"},
      ...
      el("path",{className:"spark-line",d:line,fill:"none",stroke:"url(#fcl)",strokeWidth:"3",strokeLinejoin:"round",strokeLinecap:"round"}),
...
function DesktopDashboard(props){
  ...
  return el("div",{style:{display:"flex",flexDirection:"column",gap:20}},
    ...
    props.data&&el(SubscriptionsCard,{data:props.data,categories:props.categories}),
...
const S = {
  ...
  logo:{width:42,height:42,borderRadius:14,background:"linear-gradient(135deg,var(--brand),var(--brand-2))",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 6px 18px rgba(236,59,146,.32)"},
  ...
  saveBtn:{width:"100%",padding:14,borderRadius:14,border:"none",background:"linear-gradient(135deg,var(--brand),var(--brand-2))",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",boxShadow:"0 6px 18px rgba(236,59,146,.32)"},
